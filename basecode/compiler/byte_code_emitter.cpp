...
//                        return_temp_name);
                ...
                *basic_block = epilogue_block;
                ...
                if (return_type != nullptr) {
                    auto result_temp = _variables.retain_temp(return_type->number_class());
                    vm::instruction_operand_t result_operand(assembler.make_named_ref(
                        vm::assembler_named_ref_type_t::local,
                        result_temp->name(),
                        vm::op_size_for_byte_size(target_size)));
            ...
            case element_type_t::composite_type: {
                if (!is_initialized) {
                    ...
                    for (auto fld : composite_type->fields().as_list()) {
                        ...
                        auto ctype = dynamic_cast<compiler::composite_type*>(field_var_type);
                        if (ctype != nullptr)
                    ...
                    if (padding_size > 0) {
                        basic_block->comment(
                            "padding",
                            vm::comment_location_t::after_instruction);
                        basic_block->reserve_byte(padding_size);
                    }
                }
                break;
            }
